...
char *lexer_read_identifier(Lexer *lexer, size_t *out_length);
char *lexer_read_number(Lexer *lexer, size_t *out_length);
...
Token lexer_next_token(Lexer *lexer) {
  ...
  default:
    ...
    if (is_letter(lexer->ch)) {
      ...
      char *number = lexer_read_number(lexer, &length);
      ttype = TOKEN_INT;
